...
> _Note: This was my 1st session. I have not written the core logic of the agent from scratch, but rather used this project to try to understand its working and how ADK integrates with a web server. The beautiful UI design is referred from Claude._
...
User fills form
      ↓
generate() builds a query string
      ↓
fetch("POST /api/generate", { api_key, query })
      ↓
FastAPI validates the request (Pydantic)
      ↓
ADK Agent searches Google + reasons with Gemini
      ↓
Returns { response: "## Morning\n..." }
      ↓
marked.parse() converts Markdown → HTML
      ↓
Injected into .output-body div
```
...
_(Alternatively, run `uvicorn app:app --host 0.0.0.0 --port 8000 --reload`)_
